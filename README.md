...
1. Serves the website (the HTML/CSS/JS files one folder up, in the project root).
...
The server serves website files from `..` (one level up), so keep `backend`
inside the same folder as your site files, exactly as it was unzipped.
...
your HTML files) to your server:
